...
class TestSim:
    ...
    def testServerDMP(self, destination, port):
        self.sendCMD(self.CMD_TEST_SERVER, destination, "{0}".format(chr(port)));
        
    def testClientDMP(self, source, destination, sourcePort, destinationPort, transfer):
        self.sendCMD(self.CMD_TEST_CLIENT, source, "{0}{1}{2}{3}".format(chr(destination),chr(sourcePort),chr(destinationPort),transfer));
